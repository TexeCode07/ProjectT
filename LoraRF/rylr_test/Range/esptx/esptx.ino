...
void setup() {
    ...
    sendATCommand("AT\r\n");
    sendATCommand("AT+ADDRESS=0\r\n");   // Set unique address for sender
    sendATCommand("AT+NETWORKID=5\r\n"); // Set network ID to match receiver
    sendATCommand("AT+BAND=868000000\r\n"); // Set frequency band (Adjust based on your region)
...
void sendLoRaMessage(const String &message) {
    String command = "AT+SEND=0," + String(message.length()) + "," + message + "\r\n";
